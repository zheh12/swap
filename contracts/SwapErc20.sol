...
contract SwapErc20 is Swap {
    ...
    function newSwap(address payable _receiver, bytes32 _hashlock, uint _timelock, address _tokenContract, uint256 _amount) 
        external
        payable
        verifyErc20TokenTransferable(_tokenContract, msg.sender, _amount)
        verifyTimeLockInput(_timelock) 
        returns (bytes32) {
        
        bytes32 swapId = sha256(abi.encodePacked(msg.sender, _receiver, _tokenContract, _amount, _hashlock, _timelock));
        if (haveSwap(swapId)) {
        ...
        emit NewSwap(swapId, msg.sender, _receiver, _tokenContract, _amount, _hashlock, _timelock);
    ...
    function withdraw(bytes32 _swapId, bytes32 _preimage) 
        ...
        // real transfer money
        ERC20(swap.tokenContract).transfer(swap.receiver, swap.amount);
    ...
    function rollback(bytes32 _swapId) 
        ...
        ERC20(swap.tokenContract).transfer(swap.sender, swap.amount);
